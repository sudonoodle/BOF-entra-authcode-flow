...
class EntraAuthcodeFlowBOF(BaseBOFTask):
    def __init__(self):
        super().__init__("entra-authcode-flow")

        self.parser.add_argument("clientid",
                                 help="Microsoft Entra client ID (e.g., FOCI client)")

        self.parser.add_argument("scope",
                                 help="OAuth scope (e.g., 'openid offline_access https://graph.microsoft.com/.default')")

        self.parser.add_argument("browser",
                                 type=int,
                                 choices=[0, 1],
                                 help="Browser to use: 0 = Microsoft Edge, 1 = Google Chrome")

        self.parser.add_argument("email_hint",
                                 nargs='?',
                                 default="",
                                 help="Optional email hint for login (e.g., bob@example.com)")

        self.parser.description = "Obtain Microsoft Entra tokens by initiating an OAuth authorization code flow"
...
"""
    ...
    def _encode_arguments_bof(self, arguments: List[str]) -> List[Tuple[BOFArgumentEncoding, str]]:
        # Parse arguments to get the values
        parsed = self.parser.parse_args(arguments)

        # Encode
        return [
            (BOFArgumentEncoding.WSTR, parsed.clientid),
            (BOFArgumentEncoding.WSTR, parsed.scope),
            (BOFArgumentEncoding.INT, str(parsed.browser)),
            (BOFArgumentEncoding.WSTR, parsed.email_hint)
        ]
